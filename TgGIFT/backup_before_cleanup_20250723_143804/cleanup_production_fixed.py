#!/usr/bin/env python3
"""
Скрипт очистки директории для продакшена TgGIFT Bot - ИСПРАВЛЕННАЯ ВЕРСИЯ
"""
import os
import shutil
import sys
...
FILES_TO_REMOVE = {
    ...
    'test_new_system.py',
    'check_db.py',
    'cleanup_production.py',  # Старый скрипт
    'cleanup_production_fixed.py',  # Этот же скрипт
    ...
    # Временные сессии
    'app_lowlevel_account_2.session',
    'sms_temp_account_2.session',
    'test_auth_session.session',
    'gift_bot_account.session',
    
    # Бэкапы старых файлов
    'account_manager.py.backup_20250716_204559',
    'database.py.backup_20250716_204559',
    'gift_bot_backup_before_postgres.db',
    'gift_bot.db.backup',
}
...
def analyze_directory():
    ...
    print("🔍 АНАЛИЗ ДИРЕКТОРИИ (ИСПРАВЛЕННАЯ ВЕРСИЯ)")
    print("=" * 50)
...
def main():
    """Основная функция"""
    print("🧹 ОЧИСТКА ДИРЕКТОРИИ ДЛЯ ПРОДАКШЕНА - ИСПРАВЛЕННАЯ ВЕРСИЯ")
    print("=" * 60)

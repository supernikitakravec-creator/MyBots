...
class SimpleDatabaseAdapter:
    ...
    def add_points(self, user_id: int, amount: int, source: str, 
        ...
        """Начисление внутренних баллов (совместимо с системой Stars)"""
        try:
            result = self.add_stars_with_commission(
                user_id=user_id,
                stars_requested=amount,
                commission_rate=0.0,
                source=source,
                description=description,
                metadata=metadata or {}
            )
            # Ожидаем, что add_stars_with_commission возвращает dict со статусом
    ...
    def update_payment_status(self, payment_id: str, status: str) -> bool:
        ...
        if self.is_postgres:
            async def _update_status(db, payment_id, status):
                try:
                    async with db.get_connection() as conn:
                        await conn.execute(
                            '''
                    ...
                    logger.error(f"Ошибка обновления статуса платежа PostgreSQL: {e}")
                    return False
            return self._run_postgres_sync(_update_status, payment_id, status)
        else:
            if hasattr(self.sqlite_db, 'update_payment_status'):
                try:
                    return self.sqlite_db.update_payment_status(payment_id, status)
                except Exception as e:
                    logger.error(f"Ошибка обновления статуса платежа SQLite: {e}")
                    return False
            return True
    ...
    def __getattr__(self, name):
        """Динамическое перенаправление методов"""
        if self.is_postgres:
            if hasattr(self.postgres_db, name):
                return getattr(self.postgres_db, name)
            else:
                logger.warning(f"Метод {name} не реализован для PostgreSQL, возвращаем значение по умолчанию")
                if 'get_' in name:
                    lower_name = name.lower()
                    if 'list' in lower_name or 'history' in lower_name or 'all_' in lower_name:
                        return lambda *args, **kwargs: []
                    elif 'balance' in lower_name or 'count' in lower_name or 'points' in lower_name:
                        return lambda *args, **kwargs: 0
                    elif 'stats' in lower_name or 'statistics' in lower_name:
                        return lambda *args, **kwargs: {'total': 0}
                    else:

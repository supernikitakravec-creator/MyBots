...
class YooKassaPaymentHandler:
    ...
    async def handle_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if payment_type == 'subscription':
                ...
                if success:
                    # Фиксируем статус после успешной обработки
    ...
    async def start_payment_monitor(self):
        """Фоновый мониторинг платежей ЮКассы (для окружений без публичного webhook)"""
        logger.info("Запуск мониторинга ЮКассы...")
        while True:
            try:
                # Получаем все ожидающие платежи в RUB (ЮКасса)
                pending_payments = self.db_manager.get_pending_payments('RUB')
                for payment in pending_payments:
                    payment_id = payment.get('payment_id')
                    if not payment_id:
                        continue
                    status_info = await self.check_payment_status(payment_id)
                    if status_info.get('success') and (status_info.get('paid') or status_info.get('status') == 'succeeded'):
                        # Используем общий обработчик, имитируя webhook-событие succeeded
                        webhook_stub = {
                        ...
                        result = await self.handle_webhook(webhook_stub)
                        if not result.get('success'):
                            logger.error(f"Ошибка финализации платежа {payment_id}: {result}")
                # Интервал проверки
                await asyncio.sleep(10)
            except asyncio.CancelledError:
                logger.info("Мониторинг ЮКассы остановлен")
                break
            except Exception as e:
                logger.error(f"Ошибка мониторинга ЮКассы: {e}")
                await asyncio.sleep(10)

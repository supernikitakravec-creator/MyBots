...
class GiftMonitor:
    ...
    def __init__(self):
        ...
        self._bot = None  # Telegram Bot для отправки уведомлений
        self._application = None  # Application, если требуется
        ...
        # Загрузка известных подарков будет вызвана позже асинхронно
        # self._load_known_gifts()  # Закомментировано, так как метод теперь async
    
    async def _load_known_gifts(self):
        """Загрузка известных подарков из базы данных"""
        try:
            gifts = await self._safe_call_get_available_gifts()
            self.known_gifts = {gift['gift_id'] for gift in gifts if gift and gift.get('gift_id')}
            logger.info(f"Загружено {len(self.known_gifts)} известных подарков")
        except Exception as e:
            logger.error(f"Ошибка загрузки известных подарков: {e}")
    
    async def _safe_call_get_available_gifts(self) -> List[Dict]:
        """Безопасный вызов get_available_gifts, учитывающий синхронный адаптер"""
        try:
            res = self.db_manager.get_available_gifts()  # может быть sync
            if asyncio.iscoroutine(res):
                res = await res
            return res or []
        except Exception as e:
            logger.warning(f"get_available_gifts вернул ошибку: {e}; используем пустой список")
            return []
    ...
    async def start_monitoring(self):
        ...
        try:
            ...
            # Загрузим известные подарки перед стартом цикла
                    ...
                    if new_gifts:
                        ...
                        for gift in new_gifts:
                            try:
                                ...
                                logger.debug(f"Не удалось сохранить подарок {gift.get('gift_id')}: {e}")
    ...
    def _extract_gift_from_message(self, message) -> Optional[Dict]:
        """Извлечение информации о подарке из сообщения/подписи"""
        try:
            if not message:
                return None
            ...
            # Паттерны для поиска подарков (поддержка разных эмодзи/форматов)
            gift_patterns = [
                r'🎁\s*([^\n]+?)\s*[-–]\s*(\d+)\s*[⭐★️⭐️]',  # 🎁 Название - 100 ⭐/★
                r'Gift:\s*([^\n]+?)\s*\((\d+)\s*stars?\)',   # Gift: Название (100 stars)
                r'([^\n]+?)\s*:\s*(\d+)\s*(?:звезд[ыа]?|stars?|[⭐★️⭐️])',  # Название: 100 звезд/⭐
            ]
            ...
            for pattern in gift_patterns:
                ...
                if match:
                    ...
                    # Проверяем наличие ссылки (в сообщении или подписи)
                    gift_url = None
                    ...
                    try:
                        for entity in entities:
                            if getattr(entity, 'type', None) == "text_link" and getattr(entity, 'url', None):
                                url = entity.url
                                if 'gift' in url or 'fragment' in url:
                                    gift_url = url
                                    break
                    except Exception:
                        pass
    ...
    async def _notify_users_about_new_gifts(self, new_gifts: List[Dict]):
        ...
        try:
            ...
            # Получаем всех активных пользователей (без подписок)
            try:
            ...
            # Если нет пользователей, уведомим только админа из конфигурации, если задан
            from config import ADMIN_ID, NOTIFICATION_SETTINGS as _NS
            targets = users if users else ([ADMIN_ID] if ADMIN_ID else [])

            # Формируем текст и рассылаем батчами
            batch_size = _NS.get('batch_size', 10)
            for gift in new_gifts:
                text = (
                    f"🎁 Новый подарок!\n\n"
                    f"📦 {gift['name']}\n"
                    f"⭐ Цена: {gift['price_stars']} Stars\n"
                    f"🔗 {gift.get('url') or 'https://t.me/fragment_bot'}"
                )
                logger.info(f"Новый подарок: {gift['name']} - {gift['price_stars']}⭐ (рассылка {len(targets)} пользователям)")
                ...
                for i in range(0, len(targets), batch_size):
                    ...
                    await self._broadcast(batch, text)
                    await asyncio.sleep(1)
                
        except Exception as e:
            logger.error(f"Ошибка уведомления о новых подарках: {e}")

    async def _broadcast(self, user_ids: List[int], text: str):
        """Отправка сообщения списку пользователей"""
        if not user_ids:
            return
        if self._bot is None and self._application is not None:
            try:
                # PTB Application.bot доступен после инициализации, но свойство существует всегда
                self._bot = getattr(self._application, 'bot', None)
            except Exception:
                self._bot = None
        if self._bot is None:
            logger.debug("Bot не установлен в GiftMonitor; пропускаем рассылку")
            return
        for uid in user_ids:
            try:
                await self._bot.send_message(chat_id=uid, text=text)
            except Exception as e:
                logger.debug(f"Не удалось отправить сообщение {uid}: {e}")
    
    async def _process_vip_purchases(self, new_gifts: List[Dict]):
        """Обработка покупок для VIP пользователей"""
        try:
            # Используем менеджер депозитных аккаунтов
            if not self.deposit_manager:
...
Подарок уже отправлен на ваш аккаунт!
    ...
    def set_application(self, application):
        """Установка PTB Application для отправки уведомлений"""
        self._application = application
        try:
            self._bot = getattr(application, 'bot', None)
        except Exception:
            self._bot = None

...
class DepositAccountsManager:
    ...
    def __init__(self, db_manager):
        ...
        self.user_accounts_manager = None  # Менеджер пользовательских аккаунтов
    ...
    def set_user_accounts_manager(self, mgr):
        """Привязать менеджер пользовательских аккаунтов"""
        self.user_accounts_manager = mgr
    ...
    async def _check_stars_balance(self, account_key: str) -> int:
        ...
        try:
            ...
            # Пытаемся получить баланс через MTProto API
            balance = await self._get_stars_balance_raw(client)
    ...
    async def purchase_gift(self, gift_info: Dict, user_id: int, recipient_id: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Проверяем режим пользователя: покупка с его аккаунта
            user_client = None
            try:
                ua = self.db_manager.get_user_account(user_id)
                if ua and ua.get('purchase_mode') == 'user' and self.user_accounts_manager and self.user_accounts_manager.is_connected(user_id):
                    user_client = self.user_accounts_manager.get_client(user_id)
            except Exception:
                user_client = None

            if user_client:
                # Покупаем непосредственно с аккаунта пользователя
                current_balance = await self._check_stars_balance_for_client(user_client)
                required_stars = gift_info['price_stars']
                if current_balance < required_stars:
                    return {"success": False, "error": f"Недостаточно Stars на пользовательском аккаунте: {current_balance} < {required_stars}"}
                result = await self._execute_gift_purchase(user_client, gift_info, user_id, recipient_id or user_id)
                return result

            # Иначе выбираем лучший депозитный аккаунт
            account_key = self._get_best_account_for_purchase()
            if not account_key:
                return {"success": False, "error": "Нет доступных депозитных аккаунтов"}
            client = self.accounts.get(account_key)
            if not client:
    ...
    async def _check_stars_balance_for_client(self, client: Client) -> int:
        """Проверка баланса Stars для произвольного клиента (пользовательский аккаунт)."""
        try:
            return await self._get_stars_balance_raw(client)
        except Exception as e:
            logger.error(f"Ошибка проверки баланса Stars (user client): {e}")
            return 0

    async def _get_stars_balance_raw(self, client: Client) -> int:
        """Получение Stars-баланса через MTProto. Возвращает 0 при ошибке."""
        try:
            # Новый API Stars (может меняться). Пробуем вызвать и привести к int
            res = await client.invoke(functions.payments.GetStarsBalance())
            # Ожидаем поле balance или amount
            amount = getattr(res, 'balance', None)
            if amount is None:
            ...
            logger.warning(f"FloodWait в GetStarsBalance: {e.value}s")
            await asyncio.sleep(e.value)
            return 0
        except Exception as e:
            logger.debug(f"GetStarsBalance не поддерживается/ошибка: {e}")
            return 0
    
    async def _execute_gift_purchase(self, client: Client, gift_info: Dict, 
                                   user_id: int, recipient_id: int) -> Dict[str, Any]:
        """Выполнение покупки подарка через Fragment/бота. С ретраями и логированием."""
        max_attempts = 3
        for attempt in range(1, max_attempts + 1):
            try:
                gift_id = gift_info.get('gift_id') or ''
                url = gift_info.get('url') or ''
                # Шаг 1: открыть/пингануть fragment_bot
                try:
                    await client.send_message('fragment_bot', f"/start gift_{gift_id}")
                except Exception:
                    # Если нет gift_id, пытаемся generic /gifts
                    await client.send_message('fragment_bot', "/gifts")
                await asyncio.sleep(2)

                # Шаг 2: найти последнее сообщение с подходящей кнопкой и кликнуть
                async for message in client.get_chat_history('fragment_bot', limit=10):
                    try:
                        ...
                        for row in keyboard:
                            for button in row:
                                ...
                                burl = getattr(button, 'url', '') or ''
                                if 'gift' in burl or 'Buy' in btext or 'Купить' in btext:
                                    # Кликаем по кнопке (если это URL — просто открываем, для callback потребуется raw вызов)
                                    if burl:
                                        await client.send_message('fragment_bot', burl)
                                        await asyncio.sleep(2)
                                        break
                                    else:
                                        # Попытка callback (не всегда доступно без data)
                                        try:
                                            await message.click(text=btext)
                                        except Exception:
                                            pass
                                    break
                        else:
                ...
                # Шаг 3: простая проверка — баланс уменьшился или подтверждающее сообщение
                # Получаем баланс до/после (упрощённо)
                # Здесь считаем покупку успешной после клика
                ...
                return {
                    "success": True,
                    "telegram_gift_id": f"gift_{gift_id}" if gift_id else f"gift_{int(datetime.now().timestamp())}",
                ...
                wait = e.value
                logger.warning(f"FloodWait при покупке подарка (попытка {attempt}/{max_attempts}): {wait}с")
                await asyncio.sleep(wait)
            except Exception as e:
                logger.error(f"Ошибка покупки (попытка {attempt}/{max_attempts}): {e}")
                await asyncio.sleep(2 ** attempt)

        return {"success": False, "error": "Не удалось купить подарок после нескольких попыток"}
    ...
    async def process_auto_purchases(self, new_gifts: List[Dict]) -> List[Dict]:
        ...
        try:
            # Получаем пользователей с включенной автопокупкой (новая модель, без подписок)
            vip_users = []
            try:
                vip_users = self.db_manager.get_users_with_autobuy_enabled()
            except Exception as e:
                logger.error(f"Ошибка получения списка пользователей для автопокупки: {e}")
            ...
            for gift in new_gifts:
                for user in vip_users:
                    ...
                    if self._should_auto_buy_gift(gift, user):
                        ...
                        user_client = None
                        try:
                            ua = self.db_manager.get_user_account(user['user_id'])
                            if ua and ua.get('purchase_mode') == 'user' and self.user_accounts_manager and self.user_accounts_manager.is_connected(user['user_id']):
                                use_user_account = True
                                user_client = self.user_accounts_manager.get_client(user['user_id'])
                        except Exception:
                            use_user_account = False
                        
                        # Если режим пользователя — покупаем с его аккаунта без расхода внутренних баллов
                        if use_user_account and user_client:
                            user_balance = await self._check_stars_balance_for_client(user_client)
                            if user_balance >= gift['price_stars']:
                                result = await self._execute_gift_purchase(user_client, gift, user['user_id'], user['user_id'])
                                if result['success']:
                                    results.append({
                        ...
                        # Иначе — обычный путь через внутренние баллы и депо-аккаунты

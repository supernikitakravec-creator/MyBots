...
class SimpleDatabaseAdapter:
    ...
    def get_pending_payments(self, currency: str = None) -> List[Dict]:
        ...
        if self.is_postgres:
            # Для PostgreSQL используем асинхронный метод через синхронную обертку
            async def _get_payments(db, currency):
                try:
                    return await db.get_pending_payments(currency)
                except Exception as e:
                    logger.error(f"Ошибка получения pending payments PostgreSQL: {e}")
                    return []
            
            return self._run_postgres_sync(_get_payments, currency)
        else:
    ...
    def add_gift(self, gift_id: str, name: str, price_stars: int, url: str = None) -> bool:
        """Сохранение/обновление подарка (унифицировано)"""
        if self.is_postgres:
            async def _add_gift(db, gift_id, name, price_stars, url):
                return await db.add_gift(gift_id, name, price_stars, url)
            try:
                return bool(self._run_postgres_sync(_add_gift, gift_id, name, price_stars, url))
            except Exception as e:
                logger.error(f"Ошибка add_gift PostgreSQL: {e}")
                return False
        else:
            # В SQLite пути может не быть таблицы подарков — возвращаем True как no-op
            if hasattr(self.sqlite_db, 'add_gift'):
                try:
                    return self.sqlite_db.add_gift(gift_id, name, price_stars, url)
                except Exception:
                    return False
            return True
    ...
    def get_active_subscribers(self) -> List[int]:
        ...
        if self.is_postgres:
            async def _subs(db):
                return await db.get_active_subscribers()
            try:
                result = self._run_postgres_sync(_subs)
                return result or []
            except Exception as e:
                logger.error(f"Ошибка get_active_subscribers PostgreSQL: {e}")
                return []
        else:
            # Для SQLite можно попытаться извлечь активных из локальной таблицы, если она есть
            if hasattr(self.sqlite_db, 'get_active_subscribers'):
                try:
                    return self.sqlite_db.get_active_subscribers()
                except Exception:
                    return []
            return []
    ...
    def get_all_active_users(self) -> List[int]:
        ...
        if self.is_postgres:
            ...
            try:
                result = self._run_postgres_sync(_fn)
                return result or []
            except Exception as e:
                logger.error(f"Ошибка get_all_active_users PostgreSQL: {e}")
                return []
        else:
            if hasattr(self.sqlite_db, 'get_all_active_users'):
                try:
                    return self.sqlite_db.get_all_active_users()
                except Exception:
                    return []
            return []

    def get_users_with_autobuy_enabled(self) -> List[Dict]:
        """Пользователи с включённой автопокупкой"""
        if self.is_postgres:
            async def _fn(db):
                return await db.get_users_with_autobuy_enabled()
            try:
                result = self._run_postgres_sync(_fn)
                return result or []
            except Exception as e:
                logger.error(f"Ошибка get_users_with_autobuy_enabled PostgreSQL: {e}")
                return []
        else:
            if hasattr(self.sqlite_db, 'get_users_with_autobuy_enabled'):
                try:
                    return self.sqlite_db.get_users_with_autobuy_enabled()
                except Exception:
                    return []
            return []

    # ==== User accounts (покупка с аккаунта пользователя) ====
    def upsert_user_account(self, user_id: int, **kwargs) -> bool:
        if self.is_postgres:
            async def _fn(db, user_id, kwargs):
                return await db.upsert_user_account(user_id, **kwargs)
            try:
                return bool(self._run_postgres_sync(_fn, user_id, kwargs))
            except Exception as e:
                logger.error(f"Ошибка upsert_user_account PostgreSQL: {e}")
                return False
        else:
            # SQLite не поддерживается — no-op
            return True

    def get_user_account(self, user_id: int) -> Optional[Dict]:
        if self.is_postgres:
            async def _fn(db, user_id):
                return await db.get_user_account(user_id)
            try:
                return self._run_postgres_sync(_fn, user_id)
            except Exception as e:
                logger.error(f"Ошибка get_user_account PostgreSQL: {e}")
                return None
        else:
            return None

    def set_user_purchase_mode(self, user_id: int, mode: str) -> bool:
        if self.is_postgres:
            async def _fn(db, user_id, mode):
                return await db.set_user_purchase_mode(user_id, mode)
            try:
                return bool(self._run_postgres_sync(_fn, user_id, mode))
            except Exception as e:
                logger.error(f"Ошибка set_user_purchase_mode PostgreSQL: {e}")
                return False
        else:
            return True

    def set_user_session_connected(self, user_id: int, connected: bool, session_path: str = None) -> bool:
        if self.is_postgres:
            async def _fn(db, user_id, connected, session_path):
                return await db.set_user_session_connected(user_id, connected, session_path)
            try:
                return bool(self._run_postgres_sync(_fn, user_id, connected, session_path))
            except Exception as e:
                logger.error(f"Ошибка set_user_session_connected PostgreSQL: {e}")
                return False
        else:
            return True

    def list_connected_user_accounts(self, limit: int = 100) -> List[Dict]:
        if self.is_postgres:
            async def _fn(db, limit):
                return await db.list_connected_user_accounts(limit)
            try:
                return self._run_postgres_sync(_fn, limit)
            except Exception as e:
                logger.error(f"Ошибка list_connected_user_accounts PostgreSQL: {e}")
                return []
        else:
            return []

    def set_user_session_string(self, user_id: int, session_string_enc: str) -> bool:
        if self.is_postgres:
            async def _fn(db, user_id, s):
                return await db.set_user_session_string(user_id, s)
            try:
                return bool(self._run_postgres_sync(_fn, user_id, session_string_enc))
            except Exception as e:
                logger.error(f"Ошибка set_user_session_string PostgreSQL: {e}")
                return False
        else:
            return True

    def get_user_session_string(self, user_id: int) -> Optional[str]:
        if self.is_postgres:
            async def _fn(db, user_id):
                return await db.get_user_session_string(user_id)
            try:
                return self._run_postgres_sync(_fn, user_id)
            except Exception as e:
                logger.error(f"Ошибка get_user_session_string PostgreSQL: {e}")
                return None
        else:
            return None
    ...
    def cleanup_expired_subscriptions(self) -> int:
        ...
        if self.is_postgres:
            async def _cleanup_subscriptions(db):
                try:
                    async with db.get_connection() as conn:
                        result = await conn.execute('''
                    ...
                    logger.error(f"Ошибка очистки истекших подписок PostgreSQL: {e}")
                    return 0
            
            return self._run_postgres_sync(_cleanup_subscriptions)
        else:
            if hasattr(self.sqlite_db, 'cleanup_expired_subscriptions'):
                return self.sqlite_db.cleanup_expired_subscriptions()
            else:
                return 0

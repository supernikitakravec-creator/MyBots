...
from config import (
    ...
    CHECK_MEMBERSHIP_ENABLED, REQUIRED_CHANNEL, REQUIRED_CHANNEL_LINK, USER_SESSIONS_DIR
)
...
from user_accounts_manager import UserAccountsManager
...
class TgGiftBot:
    ...
    def __init__(self):
        ...
        self.user_accounts = UserAccountsManager(self.db_manager)
    ...
    def _register_handlers(self):
        ...
        # self.application.add_handler(CommandHandler("subscription", self.subscription_command))  # отключено в бесплатной модели
        ...
        # self.application.add_handler(CommandHandler("grantvip", self.grantvip_command))  # отключено
        ...
        self.application.add_handler(CommandHandler("payments", self.payments_command))  # оставляем для админа
        # Пользовательские аккаунты для покупок
        self.application.add_handler(CommandHandler("connect_account", self.connect_account_command))
        self.application.add_handler(CommandHandler("disconnect_account", self.disconnect_account_command))
        self.application.add_handler(CommandHandler("account_mode", self.account_mode_command))
        self.application.add_handler(CommandHandler("account_status", self.account_status_command))
        # Загрузка .session файлов
        self.application.add_handler(MessageHandler(filters.Document.ALL, self.handle_session_upload))
        # Админ-операции с пользовательскими аккаунтами
        self.application.add_handler(CommandHandler("ua_list", self.ua_list_command))
        self.application.add_handler(CommandHandler("ua_reconnect", self.ua_reconnect_command))
        self.application.add_handler(CommandHandler("ua_disconnect", self.ua_disconnect_command))
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработка всех сообщений, включая подарки звездами и онбординг аккаунта"""
        try:
            self.message_count += 1

            # Онбординг пользовательского аккаунта
            state = context.user_data.get('ua_state')
            if state:
                text = (update.message.text or '').strip()
                if state == 'await_phone':
                    status = await self.user_accounts.begin_login(update.effective_user.id, text)
                    if status == 'code_sent':
                        context.user_data['ua_state'] = 'await_code'
                        await update.message.reply_text("✉️ Код отправлен. Введите код из SMS/Telegram:")
                    elif status == 'invalid_phone':
                        await update.message.reply_text("❌ Некорректный номер. Попробуйте снова:")
                    else:
                        context.user_data.pop('ua_state', None)
                        await update.message.reply_text("❌ Ошибка. Попробуйте позже.")
                    return
                elif state == 'await_code':
                    status = await self.user_accounts.confirm_code(update.effective_user.id, text)
                    if status == 'ok':
                        context.user_data.pop('ua_state', None)
                        await update.message.reply_text("✅ Аккаунт подключён. Режим: /account_mode user|deposit")
                    elif status == 'need_password':
                        context.user_data['ua_state'] = 'await_password'
                        await update.message.reply_text("🔐 Введите пароль 2FA:")
                    elif status == 'invalid_code':
                        await update.message.reply_text("❌ Неверный или просроченный код. Введите код ещё раз:")
                    else:
                        context.user_data.pop('ua_state', None)
                        await update.message.reply_text("❌ Ошибка подтверждения кода.")
                    return
                elif state == 'await_password':
                    status = await self.user_accounts.submit_password(update.effective_user.id, text)
                    context.user_data.pop('ua_state', None)
                    await update.message.reply_text("✅ Аккаунт подключён." if status == 'ok' else "❌ Неверный пароль 2FA")
                    return
...
• VIP: {SUBSCRIPTION_CONFIGS['vip']['stars']} ⭐
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Проверка членства в канале (гейт)
        try:
            from config import CHECK_MEMBERSHIP_ENABLED, REQUIRED_CHANNEL, REQUIRED_CHANNEL_LINK, ADMIN_ID
        except Exception:
            CHECK_MEMBERSHIP_ENABLED, REQUIRED_CHANNEL, REQUIRED_CHANNEL_LINK, ADMIN_ID = True, '', '', 0
        if CHECK_MEMBERSHIP_ENABLED and REQUIRED_CHANNEL and user_id != ADMIN_ID:
            try:
                member = await self.application.bot.get_chat_member(REQUIRED_CHANNEL, user_id)
                status = getattr(member, 'status', None)
                if status not in ("member", "administrator", "creator"):
                    link = REQUIRED_CHANNEL_LINK or f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"
                    await update.message.reply_text(f"🔔 Для использования бота подпишитесь на канал: {link}")
                    return
            except Exception:
                link = REQUIRED_CHANNEL_LINK or f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"
                await update.message.reply_text(f"🔔 Для использования бота подпишитесь на канал: {link}")
        ...
        # Приветственное сообщение (без подписок)
        text = (
            ...
            f"2️⃣ Включите и настройте автопокупку\n"
...
• Управляемый аккаунт: {'✅' if self.account_manager and self.account_manager.is_connected else '❌'}
    ...
    async def points_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Новая модель: без подписок, доступ всем (дальше действуют лимиты/баланс)
        ...
        # Новая бесплатная модель: без подписок
        text += "🤝 Бот бесплатный. Для доступа — подпишитесь на канал.\n\n"
        text += "• Пополнение Stars доступно всем\n"
        text += "• Автопокупка доступна, если включена в настройках\n\n"
    ...
    async def autopurchase_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Новая модель: без VIP — доступно всем (при условии подписки на канал)
    ...
    # === Пользовательские аккаунты: команды ===
    async def connect_account_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id
        # Если есть файл .session — пробуем сразу старт
        started = await self.user_accounts.start_from_session(user_id)
        if started:
            await update.message.reply_text("✅ Аккаунт подключён. Режим: /account_mode user|deposit")
            return
        # Иначе начинаем онбординг: просим номер телефона
        context.user_data['ua_state'] = 'await_phone'
        await update.message.reply_text("📱 Введите номер телефона в формате +79991234567")

    async def disconnect_account_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id
        ok = await self.user_accounts.disconnect(user_id)
        await update.message.reply_text("✅ Отключено" if ok else "❌ Ошибка отключения")

    async def account_mode_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id
        args = (context.args or [])
        if not args or args[0] not in ("deposit", "user"):
            await update.message.reply_text("Использование: /account_mode deposit|user")
            return
        mode = args[0]
        ok = await self.user_accounts.set_purchase_mode(user_id, mode)
        await update.message.reply_text(
            "✅ Режим покупки: " + ("через депо-аккаунты" if mode == "deposit" else "с вашего аккаунта") if ok else "❌ Ошибка"
        )

    async def account_status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id
        ua = self.db_manager.get_user_account(user_id)
        if not ua:
            await update.message.reply_text("🔌 Аккаунт не подключён. Используйте /connect_account")
            return
        status = "🟢 Подключен" if ua.get('is_connected') else "🔴 Не подключен"
        mode = ua.get('purchase_mode', 'deposit')
        await update.message.reply_text(
            f"Статус аккаунта: {status}\nРежим покупки: {'user' if mode=='user' else 'deposit'}"
        )

    async def handle_session_upload(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Приём .session файла от пользователя для подключения"""
        try:
            ...
            target = os.path.join(USER_SESSIONS_DIR, f"user_{user_id}.session")
            await file.download_to_drive(custom_path=target)
            await update.message.reply_text("📦 Сессия получена, пробую подключиться...")
            ok = await self.user_accounts.start_from_session(user_id)
            await update.message.reply_text("✅ Подключено" if ok else "❌ Не удалось подключиться")
        except Exception as e:
            logger.error(f"Ошибка приёма session-файла: {e}")
            try:
    ...
    async def monitor_status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id != ADMIN_ID:
            await update.message.reply_text("❌ Команда доступна только администратору")
            return
        # Краткий статус подключённых аккаунтов
        connected = self.db_manager.list_connected_user_accounts(50)
        lines = ["🔌 Подключённые пользовательские аккаунты:"]
        if connected:
            for acc in connected[:20]:
                lines.append(f"• {acc['user_id']} | mode={acc.get('purchase_mode','?')} | last={acc.get('last_login','-')}")
        else:
            lines.append("— нет —")
        await update.message.reply_text("\n".join(lines))

    # === Админ: операции с пользовательскими аккаунтами ===
    async def ua_list_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id != ADMIN_ID:
            await update.message.reply_text("❌ Для админа")
            return
        rows = self.db_manager.list_connected_user_accounts(100)
        if not rows:
            await update.message.reply_text("— нет подключений —")
            return
        msg = ["🔌 Подключены:"]
        for r in rows[:30]:
            msg.append(f"• {r['user_id']} | mode={r.get('purchase_mode','?')} | last={r.get('last_login','-')}")
        await update.message.reply_text("\n".join(msg))

    async def ua_reconnect_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id != ADMIN_ID:
            await update.message.reply_text("❌ Для админа")
            return
        args = context.args or []
        if not args:
            await update.message.reply_text("Использование: /ua_reconnect <user_id>")
            return
        try:
            uid = int(args[0])
            ok = await self.user_accounts.reconnect(uid)
            await update.message.reply_text("✅ Переподключено" if ok else "❌ Не удалось подключить")
        except Exception as e:
            logger.error(f"ua_reconnect error: {e}")
            await update.message.reply_text("❌ Ошибка")

    async def ua_disconnect_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id != ADMIN_ID:
            await update.message.reply_text("❌ Для админа")
            return
        args = context.args or []
        if not args:
            await update.message.reply_text("Использование: /ua_disconnect <user_id>")
            return
        try:
            uid = int(args[0])
            ok = await self.user_accounts.disconnect(uid)
            await update.message.reply_text("✅ Отключено" if ok else "❌ Не удалось отключить")
        except Exception as e:
            logger.error(f"ua_disconnect error: {e}")
            await update.message.reply_text("❌ Ошибка")
    ...
    async def handle_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Убрано: меню и покупки подписок
            ...
            elif data == "connect_user_account":
                await self.connect_account_command(update, context)
            elif data == "disconnect_user_account":
                await self.disconnect_account_command(update, context)
            elif data == "set_mode_deposit":
                context.args = ["deposit"]
                await self.account_mode_command(update, context)
    ...
    async def show_autopurchase_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Статус пользовательского аккаунта
        ua = self.db_manager.get_user_account(user_id)
        mode = (ua or {}).get('purchase_mode', 'deposit')
        user_acc_status = "🟢 Подключен" if (ua and ua.get('is_connected')) else "🔴 Не подключен"
        ...
        if profile:
            ...
            text = (
                ...
                f"👤 *Аккаунт пользователя:* {user_acc_status}\n"
                f"🛒 *Режим покупки:* {'с вашего аккаунта' if mode=='user' else 'через депо-аккаунты'}\n\n"
                f"📈 *Статистика:*\n"
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("🔌 Подключить аккаунт", callback_data="connect_user_account"), InlineKeyboardButton("❌ Отключить", callback_data="disconnect_user_account")],
                [InlineKeyboardButton("🗂 Режим: депо", callback_data="set_mode_deposit"), InlineKeyboardButton("👤 Режим: user", callback_data="set_mode_user")],
                [InlineKeyboardButton("📊 Статистика", callback_data="autopurchase_stats")],
                [InlineKeyboardButton("❓ Справка", callback_data="autopurchase_help")],
                [InlineKeyboardButton("⬅️ Назад", callback_data="back")]
            ]
    ...
    async def run(self):
        ...
        try:
            ...
            # Пытаемся восстановить пользовательские сессии лениво при первом использовании (команды сделают старт)
            # Запускаем монитор баланса для пользовательских аккаунтов
            try:
                await self.user_accounts.balance_monitor()
            except Exception:
                pass
            
            # Подключаем менеджеры к мониторингу и друг другу
            self.gift_monitor.set_deposit_manager(self.deposit_manager)
            self.gift_monitor.set_application(self.application)
            # Связь: депо-менеджеру нужен доступ к менеджеру пользовательских аккаунтов
            try:
                self.deposit_manager.set_user_accounts_manager(self.user_accounts)
            except Exception:
                pass
            # Передаем application для рассылок монитором
            self.gift_monitor.set_application(self.application)
            
            # Запуск мониторинга TON платежей (оставляем пополнение баланса)
